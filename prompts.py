...
2. {cuts}개의 장면(Cut)으로 나누어 각 장면의 '대사/지문(dialogue)', '상황 묘사(scene_description)', '이미지 생성용 프롬프트(image_prompt)'를 작성하세요.
3. **이미지 프롬프트(image_prompt)는 반드시 '영어(English)'로 작성해야 하며**, 구체적인 시각적 묘사(조명, 구도, 스타일 등)를 포함해야 합니다.
...
# LLM이 만든 image_prompt 뒤에 스타일 보정용 키워드를 더 붙여줍니다.
